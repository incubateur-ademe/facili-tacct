...
import { ArreteCatNat, IncendiesForet, RGA } from '@/lib/postgres/models';
...
export const GetRGA = async (
  ...
  const dbQuery = (async () => {
    try {
      console.time('Query Execution Time RGA');
      if (type === 'petr' || type === 'ept') {
        const value = await PrismaPostgres.rga.findMany({
          where: {
            [column]: libelle
          }
        });
        console.timeEnd('Query Execution Time RGA');
        return value;
      } else {
        const value = await PrismaPostgres.rga.findMany({
          where: {
