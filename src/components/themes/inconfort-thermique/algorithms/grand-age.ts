...
export const GrandAgeAlgo = (data: GrandAge[]) => {
  ...
  while (i < years.length) {
    ...
    values.forEach((el: Element) => {
      const sum_1 = Object.values(el).reduce((partialSum, a) => Number(partialSum) + Number(a), 0);
      const textColumn: string = "over_75_sum_" + year.toString();
      const percent_75_more = (100 * el[textColumn]) / Number(sum_1);
  ...
  for (let step = 0; step < Object.values(groupBy).length; step++) {
    ...
    const mean_of_percents = average(final);
