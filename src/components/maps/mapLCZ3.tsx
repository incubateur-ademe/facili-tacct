...
import styles from '../themes/inconfortThermique/themes.module.scss';
...
export const MapLCZ3 = ({
  ...
  const handleCeremaFallback = (map: maplibregl.Map) => {
    ...
    try {
      ...
      // si les layers Cerema sont déjà présentes, on les supprime
  ...
  useEffect(() => {
    ...
    // Reset fallback flag when creating new map
    ...
    // Error handlers - defined once and not dependent on state
    ...
    if (!isLczCovered) {
      ...
      // Toggle layer visibility on zoom (only if Cerema is available)
      map.on('zoom', () => {
      ...
      {isLoading ? <Loader /> : (
        <>
          ...
          <div
            ...
            style={{ width: 'auto' }}
          >
            <LegendCompColor legends={zoomMap >= 13.5 ? LczLegendOpacity70 : LczLegend} />
          </div>
        </>
      )}
    </div>
