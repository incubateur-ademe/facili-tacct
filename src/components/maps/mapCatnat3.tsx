...
export const MapCatnat2 = (props: {
  ...
  const enveloppe = useMemo(() => {
    return BoundsFromCollection(carteTerritoire, type, code);
  }, [carteTerritoire, type, code]);
  ...
  useEffect(() => {
    if (!mapContainer.current || mapLoaded === null) return;
    ...
    mapLoaded.on('load', () => {
      ...
      mapLoaded.addSource('catnat-communes', {
        type: 'geojson',
        data: geoJsonData,
        generateId: false
      });
    ...
    mapLoaded.addControl(new maplibregl.NavigationControl(), 'top-right');
    setTimeout(() => {
      setDoneLoading(true);
    }, 100);
  ...
  useEffect(() => {
    if (!mapContainer.current || mapLoaded === null || !doneLoading) return;
    // mapLoaded.addLayer({
    //   id: 'catnat-fill',
    //   type: 'fill',l
    //   source: 'catnat-communes',
    ...
    //   }
    // });
    mapLoaded.fitBounds(enveloppe as maplibregl.LngLatBoundsLike, {
      padding: 20
    });
    // mapLoaded.setPaintProperty('catnat-fill', 'fill-opacity', colorExpression);

  }, [geoJsonData, colorExpression, mapLoaded, doneLoading]);
