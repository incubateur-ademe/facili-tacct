...
const graphDataFunct = (filteredYears: string[], data: RessourcesEau[]) => {
  ...
  filteredYears.forEach((year) => {
    const genericObjects = (text: string) => data.filter(
      (item) => item.LIBELLE_SOUS_CHAMP?.includes(text)
    ).map(
      e => e[year as Years]
    ).filter(
    ...
    const obj = {
      ...
      "Industrie et autres usages économiques": Sum(genericObjects("industrie")),
      "Refroidissement des centrales électriques": Sum(genericObjects("refroidissement")),
      "Alimentation des canaux": Sum(genericObjects("alimentation")),
      "Production d'électricité (barrages hydro-électriques)": Sum(genericObjects("production")),
